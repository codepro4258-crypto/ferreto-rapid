...
if (minutes < 1) return 'just now'; if (minutes < 60) return `${minutes}m ago`; if (hours < 24) return `${hours}h ago`; if (days < 30) return `${days}d ago`; return date.toLocaleDateString(); 
}
// =========================================
...
if (geoStatus) {
geoStatus.innerHTML = <i class="fas fa-map-marker-alt text-success"></i> Location: ${position.coords.latitude.toFixed(4)}, ${position.coords.longitude.toFixed(4)} ;
}
},
(error) => {
...
const today = new Date().toISOString().split('T')[0]; const alreadyMarked = appData.attendance.some(a => a.userId === currentUser.id && a.date === today && a.status === 'Present' ); if (alreadyMarked) { if (!confirm('Attendance already marked for today. Mark again?')) { return; } } showToast('Starting attendance scanner...', 'info'); // Simulate face scanning updateAttendanceScannerStatus('Scanning', true); const startBtn = document.getElementById('btnStartAttendance'); const stopBtn = document.getElementById('btnStopAttendance'); if (startBtn && stopBtn) { startBtn.classList.add('hidden'); stopBtn.classList.remove('hidden'); } attendanceScanInterval = setTimeout(() => { markAttendanceSuccess({ latitude: 40.7128, longitude: -74.0060, accuracy: 10 }, 0.92); stopAttendanceScanner(); }, 2000); 
}
...
const attendanceRecord = { id: Date.now(), userId: currentUser.id, courseId: currentUser.courseId, date: dateStr, time: timeStr, lat: coords.latitude, lng: coords.longitude, confidence: Math.round(confidence * 100) / 100, status: 'Present', method: 'FaceSafe Ultra Biometric', notes: 'Automated face verification', verified: true, device: navigator.userAgent }; appData.attendance.push(attendanceRecord); saveAppData(); showToast(`Attendance marked successfully at ${timeStr}`, 'success'); logSystem('ATTENDANCE', `Marked attendance with ${Math.round(confidence * 100)}% confidence`, currentUser.id); updateAttendanceBadge(); loadAttendanceHistory(); refreshDashboard(); setTimeout(() => { alert(`✅ Attendance Confirmed!\n\nDate: ${dateStr}\nTime: ${timeStr}\nConfidence: ${Math.round(confidence * 100)}%`); }, 500); 
}
...
// Sort by date (newest first) records.sort((a, b) => { const dateA = new Date(`${a.date}T${a.time || '00:00:00'}`); const dateB = new Date(`${b.date}T${b.time || '00:00:00'}`); return dateB - dateA; }); if (records.length === 0) { tbody.innerHTML = ` <tr> <td colspan="6" class="text-center py-8"> <i class="fas fa-calendar-times fa-2x mb-4 text-gray-300"></i> <p class="text-gray-500">No attendance records found.</p> <button class="btn btn-primary mt-4" onclick="startAttendanceScanner()"> <i class="fas fa-camera"></i> Mark First Attendance </button> </td> </tr> `; return; } tbody.innerHTML = ''; records.forEach(record => { const course = appData.courses.find(c => c.id == record.courseId); const locStr = record.lat && record.lng ? `${record.lat.toFixed(4)}, ${record.lng.toFixed(4)}` : 'N/A'; const confidence = record.confidence ? `<span class="font-bold ${record.confidence > 0.9 ? 'text-success' : record.confidence > 0.7 ? 'text-warning' : 'text-danger'}"> ${Math.round(record.confidence * 100)}% </span>` : 'N/A'; const statusClass = record.status === 'Present' ? 'badge-success' : record.status === 'Absent' ? 'badge-danger' : record.status === 'Late' ? 'badge-warning' : 'badge-info'; tbody.innerHTML += ` <tr> <td> <div class="font-medium">${record.date}</div> <div class="text-xs text-gray">${record.time || ''}</div> </td> <td>${course ? course.name : 'General'}</td> <td> <div class="text-sm">${locStr}</div> ${record.locationAccuracy ? `<div class="text-xs text-gray">±${Math.round(record.locationAccuracy)}m</div>` : ''} </td> <td><span class="badge badge-info">${record.method}</span></td> <td>${confidence}</td> <td><span class="badge ${statusClass}">${record.status}</span></td> </tr> `; }); 
}
function exportAttendanceCSV() {
const records = appData.attendance.filter(a => a.userId === currentUser.id);
if (records.length === 0) { showToast('No attendance records to export', 'warning'); return; } let csv = 'Date,Time,Course,Latitude,Longitude,Method,Confidence,Status\n'; records.forEach(record => { const course = appData.courses.find(c => c.id == record.courseId); const courseName = course ? course.name : 'General'; csv += `"${record.date}","${record.time || ''}","${courseName}",`; csv += `${record.lat || ''},${record.lng || ''},"${record.method}",`; csv += `${record.confidence || ''},"${record.status}"\n`; }); const blob = new Blob([csv], { type: 'text/csv' }); const url = URL.createObjectURL(blob); const a = document.createElement('a'); a.href = url; a.download = `attendance_${currentUser.username}_${new Date().toISOString().split('T')[0]}.csv`; document.body.appendChild(a); a.click(); document.body.removeChild(a); URL.revokeObjectURL(url); showToast('Attendance data exported as CSV', 'success'); logSystem('EXPORT', 'Exported attendance CSV', currentUser.id); 
}
// =========================================
// 11. ADMIN FUNCTIONS
// =========================================
...
courseSelect.innerHTML = '<option value="">No Course</option>' + appData.courses.map(c => `<option value="${c.id}">${c.code} - ${c.name}</option>` ).join(''); tbody.innerHTML = ''; appData.users.forEach(u => { const course = appData.courses.find(c => c.id == u.courseId); const hasFace = u.faceDescriptor ? true : false; const lastLogin = u.lastLogin ? new Date(u.lastLogin).toLocaleDateString() : 'Never'; tbody.innerHTML += ` <tr> <td> <div class="flex items-center gap-3"> <div class="user-avatar" style="width: 36px; height: 36px; font-size: 0.9rem;"> ${u.name.charAt(0).toUpperCase()} </div> <div> <div class="font-bold text-sm">${u.name}</div> <div class="text-xs text-gray">${u.email || 'No email'}</div> </div> </div> </td> <td>${u.username}</td> <td><span class="badge badge-primary">${u.role}</span></td> <td>${course ? course.name : '<span class="text-gray">None</span>'}</td> <td> <div class="flex flex-col gap-1"> <span class="badge ${hasFace ? 'badge-success' : 'badge-warning'}"> <i class="fas fa-${hasFace ? 'check' : 'times'}"></i> ${hasFace ? 'Registered' : 'Not Set'} </span> </div> </td> <td class="text-sm text-gray">${lastLogin}</td> <td> <div class="flex gap-1"> <button class="btn btn-sm btn-secondary" onclick="openFaceRegistration(${u.id})" title="Register Face"> <i class="fas fa-user-shield"></i> </button> <button class="btn btn-sm btn-outline" onclick="editUser(${u.id})"> <i class="fas fa-edit"></i> </button> ${u.id !== currentUser.id ? ` <button class="btn btn-sm btn-danger" onclick="deleteUser(${u.id})"> <i class="fas fa-trash"></i> </button> ` : ` <span class="text-xs text-gray px-2">Current</span> `} </div> </td> </tr> `; }); 
}
function loadAdminCourses() {
const grid = document.getElementById('adminCoursesGrid');
const courses = appData.courses;
if (courses.length === 0) { grid.innerHTML = ` <div class="empty-state w-full"> <i class="fas fa-graduation-cap"></i> <h3>No Courses</h3> <p>No courses have been created yet.</p> <button class="btn btn-primary mt-4" onclick="openCourseModal()"> <i class="fas fa-plus"></i> Create First Course </button> </div> `; return; } grid.innerHTML = ''; courses.forEach(course => { const studentCount = appData.users.filter(u => u.courseId == course.id && u.role === 'student' ).length; const materialCount = appData.materials.filter(m => m.courseId == course.id ).length; const card = document.createElement('div'); card.className = 'card'; card.innerHTML = ` <div class="card-header"> <div> <div class="badge badge-primary mb-2">${course.code}</div> <div class="card-title">${course.name}</div> <div class="card-subtitle">${course.lecturer}</div> </div> <div class="text-right"> <div class="text-sm text-gray">${studentCount} students</div> <div class="text-xs text-gray">${materialCount} materials</div> </div> </div> <div class="card-body"> <p class="text-sm text-gray-600 mb-3 line-clamp-2">${course.description || 'No description'}</p> <div class="flex flex-col gap-1 text-sm"> <div class="flex items-center gap-2"> <i class="fas fa-clock text-gray-400"></i> <span>${course.schedule || 'Schedule not set'}</span> </div> <div class="flex items-center gap-2"> <i class="fas fa-star text-gray-400"></i> <span>${course.credits || 0} credits</span> </div> </div> </div> <div class="card-footer"> <div class="flex gap-2"> <button class="btn btn-sm btn-outline" onclick="editCourse(${course.id})"> <i class="fas fa-edit"></i> Edit </button> </div> <button class="btn btn-sm btn-danger" onclick="deleteCourse(${course.id})"> <i class="fas fa-trash"></i> </button> </div> `; grid.appendChild(card); }); 
}
function loadAdminMaterials() {
const tbody = document.getElementById('adminMaterialsTable');
const materials = appData.materials;
if (materials.length === 0) { tbody.innerHTML = ` <tr> <td colspan="7" class="text-center py-8"> <i class="fas fa-file-alt fa-2x mb-4 text-gray-300"></i> <p class="text-gray-500">No materials available.</p> <button class="btn btn-primary mt-4" onclick="openMaterialModal()"> <i class="fas fa-plus"></i> Add First Material </button> </td> </tr> `; return; } tbody.innerHTML = ''; materials.forEach(material => { const course = appData.courses.find(c => c.id == material.courseId); const typeBadge = material.type === 'pdf' ? 'badge-danger' : material.type === 'code' ? 'badge-primary' : material.type === 'video' ? 'badge-info' : 'badge-gray'; const typeIcon = material.type === 'pdf' ? 'fa-file-pdf' : material.type === 'code' ? 'fa-code' : material.type === 'video' ? 'fa-file-video' : material.type === 'doc' ? 'fa-file-word' : material.type === 'link' ? 'fa-link' : 'fa-file'; tbody.innerHTML += ` <tr> <td class="font-medium">${material.title}</td> <td> <span class="badge ${typeBadge}"> <i class="fas ${typeIcon}"></i> ${material.type.toUpperCase()} </span> </td> <td>${course ? course.code : 'General'}</td> <td class="text-sm text-gray">${material.fileSize || 'N/A'}</td> <td class="text-sm text-gray">${material.date}</td> <td class="text-center">${material.downloads || 0}</td> <td> <div class="flex gap-1"> <button class="btn btn-sm btn-outline" onclick="editMaterial(${material.id})"> <i class="fas fa-edit"></i> </button> <button class="btn btn-sm btn-danger" onclick="deleteMaterial(${material.id})"> <i class="fas fa-trash"></i> </button> </div> </td> </tr> `; }); 
}
...
if (!name || !email || !username) { showToast('Please fill all required fields', 'error'); return; } const emailExists = appData.users.some(u => u.email === email && String(u.id) !== String(userId)); const usernameExists = appData.users.some(u => u.username === username && String(u.id) !== String(userId)); if (emailExists || usernameExists) { showToast('Email or username already exists', 'error'); return; } if (!userId && (!password || password.length < 8)) { showToast('Password must be at least 8 characters', 'error'); return; } if (userId) { const userIndex = appData.users.findIndex(u => String(u.id) === String(userId)); if (userIndex === -1) return; const existing = appData.users[userIndex]; const oldCourseId = existing.courseId; const updatedUser = { ...existing, name, email, username, role, courseId: courseId ? Number(courseId) : null, notes, password: password ? password : existing.password }; appData.users[userIndex] = updatedUser; syncCourseMembership(existing.id, oldCourseId, courseId ? Number(courseId) : null); if (currentUser && currentUser.id === updatedUser.id) { currentUser = { ...updatedUser }; sessionStorage.setItem('currentUser', JSON.stringify(currentUser)); showDashboard(); } showToast('User updated successfully', 'success'); logSystem('USER_UPDATE', `Updated user: ${name}`, currentUser.id); } else { const newUser = { id: Date.now(), username, password, email, name, role, courseId: courseId ? Number(courseId) : null, faceDescriptor: null, createdAt: new Date().toISOString(), lastLogin: null, status: 'active', preferences: {}, likedProjects: [], notes }; appData.users.push(newUser); syncCourseMembership(newUser.id, null, newUser.courseId); showToast('User created successfully', 'success'); logSystem('USER_CREATE', `Created user: ${name}`, currentUser.id); } saveAppData(); closeModal('userModal'); loadAdminUsers(); refreshDashboard(); 
}
function deleteUser(userId) {
const user = appData.users.find(u => u.id === userId);
if (!user) return;
if (!confirm(Delete user ${user.name}? This action cannot be undone.)) return;
appData.users = appData.users.filter(u => u.id !== userId); appData.attendance = appData.attendance.filter(a => a.userId !== userId); appData.projects = appData.projects.filter(p => p.userId !== userId); appData.groupMessages = appData.groupMessages.filter(m => m.userId !== userId); appData.groups = appData.groups.map(group => ({ ...group, memberIds: group.memberIds.filter(id => id !== userId) })); syncCourseMembership(userId, user.courseId, null); saveAppData(); showToast('User deleted successfully', 'success'); logSystem('USER_DELETE', `Deleted user: ${user.name}`, currentUser.id); loadAdminUsers(); refreshDashboard(); 
}
...
if (!code || !name || !lecturer) { showToast('Please fill all required course fields', 'error'); return; } const codeExists = appData.courses.some(c => c.code === code && String(c.id) !== String(courseId)); if (codeExists) { showToast('Course code already exists', 'error'); return; } if (courseId) { const index = appData.courses.findIndex(c => String(c.id) === String(courseId)); if (index === -1) return; appData.courses[index] = { ...appData.courses[index], code, name, lecturer, schedule, description, credits, updatedAt: new Date().toISOString() }; showToast('Course updated successfully', 'success'); logSystem('COURSE_UPDATE', `Updated course: ${code}`, currentUser.id); } else { appData.courses.push({ id: Date.now(), code, name, lecturer, schedule, credits, description, students: [], materials: [], createdAt: new Date().toISOString() }); showToast('Course created successfully', 'success'); logSystem('COURSE_CREATE', `Created course: ${code}`, currentUser.id); } saveAppData(); closeModal('courseModal'); loadAdminCourses(); loadAdminUsers(); loadStudentMaterials(); 
}
function deleteCourse(courseId) {
const course = appData.courses.find(c => c.id === courseId);
if (!course) return;
if (!confirm(Delete course ${course.name}?)) return;
appData.courses = appData.courses.filter(c => c.id !== courseId); appData.users = appData.users.map(user => ( user.courseId === courseId ? { ...user, courseId: null } : user )); appData.materials = appData.materials.map(material => ( material.courseId === courseId ? { ...material, courseId: null } : material )); appData.attendance = appData.attendance.filter(record => record.courseId !== courseId); saveAppData(); showToast('Course deleted', 'success'); logSystem('COURSE_DELETE', `Deleted course: ${course.code}`, currentUser.id); loadAdminCourses(); loadAdminUsers(); loadAdminMaterials(); refreshDashboard(); 
}
...
const user = appData.users.find(u => u.id === userId);
if (!user) return;
registrationUserId = userId; registrationSamples = 0; updateRegistrationUI('Idle', 0); const nameEl = document.getElementById('regUserName'); if (nameEl) nameEl.textContent = user.name; const samplesGrid = document.getElementById('registrationSamplesGrid'); if (samplesGrid) { samplesGrid.innerHTML = ''; samplesGrid.style.display = 'none'; } const preview = document.getElementById('regFacePreview'); const icon = document.getElementById('regFaceIcon'); if (preview) preview.classList.add('hidden'); if (icon) icon.classList.remove('hidden'); openModal('faceRegistrationModal'); 
}
...
const totalSamples = ENTERPRISE_CONFIG.FACE.REG_FRAMES; updateRegistrationUI('Capturing', 0); document.getElementById('btnStartReg').classList.add('hidden'); document.getElementById('btnStopReg').classList.remove('hidden'); registrationInterval = setInterval(() => { registrationSamples += 1; const progress = Math.min(100, Math.round((registrationSamples / totalSamples) * 100)); updateRegistrationUI('Capturing', progress); if (registrationSamples >= totalSamples) { finalizeFaceRegistration(); } }, 140); 
}
...
if (regStatusText) regStatusText.textContent = status; if (regStatusDot) regStatusDot.classList.toggle('active', status === 'Capturing'); if (regHudSamples) regHudSamples.textContent = `SAMPLES: ${registrationSamples}/${ENTERPRISE_CONFIG.FACE.REG_FRAMES}`; if (regProgressWrap) regProgressWrap.classList.toggle('hidden', progress === 0); if (regProgressBar) regProgressBar.style.width = `${progress}%`; 
}
...
const userIndex = appData.users.findIndex(u => u.id === registrationUserId); if (userIndex !== -1) { appData.users[userIndex].faceDescriptor = `face_${registrationUserId}_${Date.now()}`; appData.users[userIndex].updatedAt = new Date().toISOString(); } saveAppData(); updateRegistrationUI('Completed', 100); document.getElementById('btnStartReg').classList.remove('hidden'); document.getElementById('btnStopReg').classList.add('hidden'); showToast('Face registration completed', 'success'); loadAdminUsers(); updateAttendanceFaceStatus(); 
}
// =========================================
// 13. GROUP CHAT
// =========================================
...
if (!filtered.length) { listEl.innerHTML = ` <div class="empty-state"> <i class="fas fa-users"></i> <h3>No groups found</h3> <p>${groupFilter ? 'Try a different search.' : 'Ask an admin to create a group.'}</p> </div> `; currentGroupId = null; renderGroupMessages(); return; } if (!currentGroupId || !filtered.some(group => group.id === currentGroupId)) { currentGroupId = filtered[0].id; } listEl.innerHTML = ''; filtered.forEach(group => { const memberCount = group.memberIds.length; const isActive = group.id === currentGroupId; const item = document.createElement('div'); item.className = `group-list-item ${isActive ? 'active' : ''}`; item.onclick = () => selectGroup(group.id); item.innerHTML = ` <div class="group-list-title">${group.name}</div> <div class="group-list-meta">${memberCount} members</div> `; listEl.appendChild(item); }); 
}
...
if (!messageEl || !headerEl) return; if (!group) { messageEl.innerHTML = ` <div class="empty-state"> <i class="fas fa-comments"></i> <h3>No group selected</h3> <p>Select a group from the left panel to view messages.</p> </div> `; headerEl.innerHTML = ` <div> <h3>Select a group</h3> <p class="text-sm text-gray">Choose a group to start chatting.</p> </div> `; if (memberList) memberList.innerHTML = ''; if (input) input.disabled = true; return; } if (input) input.disabled = false; headerEl.innerHTML = ` <div> <h3>${group.name}</h3> <p class="text-sm text-gray">${group.description || 'Collaboration group'}</p> </div> `; if (memberList) { const members = group.memberIds.map(id => appData.users.find(u => u.id === id)).filter(Boolean); memberList.innerHTML = members.slice(0, 4).map(member => ` <span class="member-chip">${member.name.split(' ')[0]}</span> `).join(''); if (members.length > 4) { memberList.innerHTML += `<span class="member-chip">+${members.length - 4}</span>`; } } const messages = appData.groupMessages .filter(message => message.groupId === group.id) .sort((a, b) => new Date(a.createdAt) - new Date(b.createdAt)); if (!messages.length) { messageEl.innerHTML = ` <div class="empty-state"> <i class="fas fa-comment-dots"></i> <h3>No messages yet</h3> <p>Be the first to share an update.</p> </div> `; return; } messageEl.innerHTML = messages.map(message => { const author = appData.users.find(u => u.id === message.userId); const isCurrent = message.userId === currentUser.id; const time = new Date(message.createdAt).toLocaleTimeString([], { hour: '2-digit', minute: '2-digit' }); return ` <div class="chat-message ${isCurrent ? 'self' : ''}"> <div class="chat-message-avatar">${author ? author.name.charAt(0).toUpperCase() : '?'}</div> <div class="chat-message-content"> <div class="chat-message-header"> <span class="chat-message-author">${author ? author.name : 'Unknown'}</span> <span class="chat-message-time">${time}</span> </div> <div class="chat-message-body">${formatChatMessage(message.content)}</div> </div> </div> `; }).join(''); messageEl.scrollTop = messageEl.scrollHeight; 
}
...
appData.groupMessages.push({ id: Date.now(), groupId: group.id, userId: currentUser.id, content: message, createdAt: new Date().toISOString() }); saveAppData(); input.value = ''; renderGroupMessages(); logSystem('GROUP_CHAT', `Sent message in ${group.name}`, currentUser.id); 
}
...
if (!name) { showToast('Group name is required', 'error'); return; } const memberIds = Array.from(new Set([currentUser.id, ...selectedMembers])); if (groupId) { const index = appData.groups.findIndex(group => String(group.id) === String(groupId)); if (index === -1) return; appData.groups[index] = { ...appData.groups[index], name, description, memberIds, updatedAt: new Date().toISOString() }; showToast('Group updated', 'success'); logSystem('GROUP_UPDATE', `Updated group: ${name}`, currentUser.id); } else { appData.groups.push({ id: Date.now(), name, description, createdBy: currentUser.id, memberIds, createdAt: new Date().toISOString(), updatedAt: new Date().toISOString() }); showToast('Group created', 'success'); logSystem('GROUP_CREATE', `Created group: ${name}`, currentUser.id); } saveAppData(); closeModal('groupModal'); renderGroupList(); 
}
...
container.innerHTML = members.map(user => <label class="member-select-item"> <input type="checkbox" name="groupMember" value="${user.id}" ${selectedIds.includes(user.id) ? 'checked' : ''}> <span>${user.name} <span class="text-xs text-gray">(${user.role})</span></span> </label> ).join('');
}
...
if (!grid) return;
if (!projects.length) {
grid.innerHTML = <div class="empty-state w-full"> <i class="fas fa-trophy"></i> <h3>No projects available</h3> <p>Projects will appear here once students publish them.</p> </div> ;
return;
}
grid.innerHTML = '';
...
card.className = 'card';
card.innerHTML = <div class="card-header"> <div> <div class="card-title">${project.name}</div> <div class="card-subtitle">${author ? author.name : 'Unknown author'}</div> </div> <span class="badge badge-primary">${project.category}</span> </div> <div class="card-body"> <p class="text-sm text-gray-600 mb-3 line-clamp-2">${project.description || 'No description'}</p> <div class="flex items-center gap-4 text-sm text-gray"> <div class="flex items-center gap-1"><i class="fas fa-heart"></i> ${project.likes || 0}</div> <div class="flex items-center gap-1"><i class="fas fa-eye"></i> ${project.views || 0}</div> <div class="flex items-center gap-1"><i class="fas fa-code-branch"></i> ${project.forks || 0}</div> </div> </div> <div class="card-footer"> <button class="btn btn-sm btn-outline" onclick="viewProject(${project.id})"> <i class="fas fa-eye"></i> View </button> </div> ;
...
document.getElementById('adminTotalUsers').textContent = totalUsers; document.getElementById('adminUsersTrend').textContent = `Active today: ${activeToday}`; document.getElementById('adminActiveSessions').textContent = activeSessions.size; document.getElementById('adminAttendanceRate').textContent = `${attendanceRate}%`; document.getElementById('adminAttendanceTrend').textContent = `Today: ${todayAttendance.length}/${totalStudents}`; document.getElementById('adminStorageUsage').textContent = `${calculateStorageUsage().toFixed(2)} MB`; const table = document.getElementById('adminActivitiesTable'); const logs = appData.systemLogs.slice(0, 8); if (!logs.length) { table.innerHTML = ` <tr> <td colspan="5" class="text-center text-gray py-6">No recent activity.</td> </tr> `; return; } table.innerHTML = logs.map(log => { const user = appData.users.find(u => u.id === log.userId); return ` <tr> <td>${new Date(log.timestamp).toLocaleString()}</td> <td>${user ? user.name : 'System'}</td> <td>${log.action}</td> <td>${log.details}</td> <td>${log.ip}</td> </tr> `; }).join(''); 
}
...
const activeLink = document.querySelector(`.menu-item[data-section="${sectionId}"]`); if (activeLink) { activeLink.classList.add('active'); } // Hide all sections, show active document.querySelectorAll('.content-section').forEach(section => { section.classList.remove('active'); }); const activeSection = document.getElementById(sectionId); if (activeSection) { activeSection.classList.add('active'); loadSectionData(sectionId); } 
}
...
document.querySelectorAll('.menu-item').forEach(item => { item.addEventListener('click', (e) => { e.preventDefault(); const section = item.getAttribute('data-section'); showSection(section); document.getElementById('sidebar').classList.remove('mobile-open'); }); }); 
}
function toggleUserDropdown() {
const dropdown = document.getElementById('userDropdown');
dropdown.classList.toggle('show');
document.addEventListener('click', function closeDropdown(e) { if (!dropdown.contains(e.target) && e.target.id !== 'userAvatar') { dropdown.classList.remove('show'); document.removeEventListener('click', closeDropdown); } }); 
}
...
if (!container) return;
const toast = document.createElement('div'); toast.className = `toast ${type}`; let icon = 'fa-check-circle'; if (type === 'error') icon = 'fa-exclamation-circle'; if (type === 'warning') icon = 'fa-exclamation-triangle'; if (type === 'info') icon = 'fa-info-circle'; toast.innerHTML = ` <div class="toast-icon"> <i class="fas ${icon}"></i> </div> <div class="flex-1"> <div class="font-medium">${message}</div> </div> `; container.appendChild(toast); setTimeout(() => { toast.style.animation = 'slideInRight 0.3s ease reverse'; setTimeout(() => { if (toast.parentNode) { toast.remove(); } }, 300); }, 5000); 
}
...
appData.systemLogs.unshift(log); if (appData.systemLogs.length > 1000) { appData.systemLogs = appData.systemLogs.slice(0, 1000); } saveAppData(); return log; 
}
// =========================================
...
// Modal forms document.getElementById('userForm').addEventListener('submit', (e) => { e.preventDefault(); handleSaveUser(e); }); document.getElementById('courseForm').addEventListener('submit', (e) => { e.preventDefault(); handleSaveCourse(e); }); document.getElementById('materialForm').addEventListener('submit', handleSaveMaterial); document.getElementById('projectForm').addEventListener('submit', handleSaveProject); const groupForm = document.getElementById('groupForm'); if (groupForm) { groupForm.addEventListener('submit', handleSaveGroup); } const groupMessageInput = document.getElementById('groupMessageInput'); if (groupMessageInput) { groupMessageInput.addEventListener('keydown', (event) => { if ((event.ctrlKey || event.metaKey) && event.key === 'Enter') { sendGroupMessage(); } }); } // Close modals on overlay click document.querySelectorAll('.modal-overlay').forEach(overlay => { overlay.addEventListener('click', function(e) { if (e.target === this) { this.classList.remove('open'); } }); }); // Escape key closes modals document.addEventListener('keydown', function(e) { if (e.key === 'Escape') { document.querySelectorAll('.modal-overlay.open').forEach(modal => { modal.classList.remove('open'); }); } }); // Activity monitoring document.addEventListener('mousemove', updateActivity); document.addEventListener('keypress', updateActivity); // File upload drag and drop const dropArea = document.getElementById('fileDropArea'); if (dropArea) { ['dragenter', 'dragover', 'dragleave', 'drop'].forEach(eventName => { dropArea.addEventListener(eventName, preventDefaults, false); }); function preventDefaults(e) { e.preventDefault(); e.stopPropagation(); } ['dragenter', 'dragover'].forEach(eventName => { dropArea.addEventListener(eventName, highlight, false); }); ['dragleave', 'drop'].forEach(eventName => { dropArea.addEventListener(eventName, unhighlight, false); }); function highlight() { dropArea.classList.add('dragover'); } function unhighlight() { dropArea.classList.remove('dragover'); } dropArea.addEventListener('drop', handleDrop, false); function handleDrop(e) { const dt = e.dataTransfer; const files = dt.files; handleFileUpload(files); } } 
}